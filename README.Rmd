...
---
title: `r if(word_out) '"NAFOdown: An R Markdown Template for Producing NAFO Scientific Council Documents"'`
author: `r if(word_out) '"Paul M. Regular^1^, Bob Rodgers^1^, Laura Wheeland^1^, Sean C. Anderson^2^"'`
date: |
    `r if(word_out) "^1^Northwest Atlantic Fisheries Centre, Fisheries and Oceans Canada, P.O.Box 5667, St. John’s, NL, A1C 5X1, Canada\\"`
    `r if(word_out) "^2^Pacific Biological Station, Fisheries and Oceans Canada, 3190 Hammond Bay Rd., Nanaimo, British Columbia, V9T 6N7, Canada"`
---


`r if (!word_out) "[![Travis-CI Build Status](https://travis-ci.org/nafc-assess/NAFOdown.svg?branch=master)](https://travis-ci.org/nafc-assess/NAFOdown)"`
...
A skeleton of a SCR or STACFIS document will be produced and stored in your working directory by running this code:
